...
import (
	...
	v1beta1 "github.com/wd/kata-csi/pkg/cnfs/v1beta"
...
func (ns *nodeServer) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	if opt.UseSharedPath {
		...
		if IsOssfsMounted(sharedPath) {
			...
			if err := utils2.CreateDest(sharedPath); err != nil {
			...
			if err := utils2.DoMountInHost(mntCmd); err != nil {
				return nil, err
			}
		}
		...
		// Create Mount Path
		if err := utils2.CreateDest(mountPath); err != nil {
		...
		utils2.WriteMetricsInfo(metricsPathPrefix, req, opt.MetricsTop, OssFsType, "oss", opt.Bucket)
		if err := utils2.DoMountInHost(mntCmd); err != nil {
			return nil, err
		}
	}
...
func saveOssfsCredential(options *Options) error {
	oldContentByte := []byte{}
	if utils2.IsFileExisting(OssfsCredentialFile) {
	...
	if err := utils2.WriteAndSyncFile(OssfsCredentialFile, []byte(newContentStr), 0640); err != nil {
...
func uniqOssfsCredential() {
	...
	if err := utils2.WriteAndSyncFile(OssfsCredentialFile, []byte(uniqOssInfoStr), 0640); err != nil {
...
func (ns *nodeServer) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	if _, err := utils2.Run(umntCmd); err != nil {
		log.Errorf("Umount oss fail, with: %s", err.Error())
		return nil, errors.New("Oss, Umount oss Fail: " + err.Error())
	}
